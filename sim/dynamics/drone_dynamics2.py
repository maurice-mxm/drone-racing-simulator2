...
def dynamics(state, control, old_control):
    ...
    TIMESTEP = 0.02  # Time step for the integration and derivation (s)
    ...
    torque_x = (-torques[0] - torques[1] + torques[2] + torques[3])
    torque_y = (-torques[0] + torques[1] + torques[2] - torques[3])
    ...
    acc = quaternion_rotate(quaternion, (force + np.dot(DRAG_COEFF, velocity) * np.sum(rot_vel)) / M) + G
